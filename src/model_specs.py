...
Defines extensible observation and action space specifications that allow different models to use different observation/action formats while competing in the same environment.
"""
...
class ObservationGenerator(ABC):
    ...
    def _get_action_mask(self, self, ship: dict, is_player: bool = True) -> np.ndarray:
        ...
        for action in range(num_actions):
            is_valid, _ = self.env._action_valid_for_state(action, ship, is_player=is_player)
            if is_valid:
                mask[action] = 1
        return mask
...
class FullObservationGenerator(ObservationGenerator):
    ...
    def generate(self, ship: dict) -> Dict[str, np.ndarray]:
        ...
        original_flag = getattr(self.env, 'generating_observation', False)
        self.env.generating_observation = True
        try:
            ...
            self.env.generating_observation = original_flag
