...
class ActionMaskWrapper(gym.Wrapper):
    ...
    Standard PPO ignores the action mask in the observation dict. This wrapper intercepts invalid actions in `step()`, replaces them with a randomly-sampled *valid* action, and applies a penalty so the model learns to avoid masked actions.

    The penalty-based approach is less efficient than true masked sampling,
    but is significantly better than the silent enforcement fallback that existed before, because:
        - The model receives a consistent negative reward for invalid choices.
        - The replacement action is random among valid ones (no bias toward a single fallback), so the model can't game the fallback.
    ...
    def step(self, action):
        ...
        # ([action_type, target_x, target_y, energy_bin]); the action mask applies only to the action_type component (index 0). Support both array actions and bare scalar actions for backward compatibility.
    ...
    def _on_step(self) -> bool:
        try:
            ...
            if progress_milestone > self.last_reported_progress and progress_milestone % 10 == 0:
                ...
                print(f"\n[progress_milestone] Progress: {self.num_timesteps}/{self.total_timesteps} timesteps | Episodes: {len(self.episode_credits)} | Avg Credits (last 10): {avg_credits:.1f}")
...
def safe_model_save(model, path):
    ...
    SB3's model.save() internally calls platform.platform() to log system info. On some corporate/restricted Windows environments this subprocess call fails. We monkey-patch platform.platform temporarily to return a safe string."""
...
def _print_performance_participant_stats(stats_list, opponents):
    ...
    for stats in stats_list:
        episode_rankings = []
        episode_rankings.append(('PLAYER', 
...
if is_transfer:
    ...
    try:
        ...
        print(f"x  Error loading model from {model_path_with_zip}: {e}")
