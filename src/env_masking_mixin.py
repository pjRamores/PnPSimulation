...
class EnvMaskingMixin:
    ...
    def _normalize_action(self, action) -> Tuple[int, Optional[Tuple[int, int]], Optional[int]]:
        ...
        preserving the legacy auto-target / default-energy behaviour relied on by
        the heuristic AIs and scalar-Discrete callers. A structured action (a dict matching the
...
def _build_mask_state(self, ship: dict, is_player: bool = True) -> 'action_masker.MaskState':
    ...
    Resolves the candidate ATTACK/RAISE_SHIELDS/PLUNDER target list the same way the validity rules historically did:
    the player targets opponents; an opponent targets the player plus the other opponents.
    """
    if is_player:
