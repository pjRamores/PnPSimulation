...
class EnvActionsMixin:
    ...
    def _execute_action(self, action: int, ship: dict, is_player: bool = True,
        ...
        'target' (an explicit (x, y) coordinate) and 'energy' (an explicit
        energy/payload amount) come from the structured action space. When `None`
        the action falls back to its automatic target selection / default energy,
        preserving the behaviour used by the heuristic AIs and legacy scalar-action
        callers.
        """
        reward = 0.0
