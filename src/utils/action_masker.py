...
Single source of truth for the game's per-state action validity rules,
the boolean action mask, and the "force an invalid action to a valid one"
enforcement that the environment and the model-backed bot both need.

The utility is intentionally **self-contained** -- it imports only numpy and
the standard library, with no dependency on the environment package. Callers adapt
their own state (the full simulator state, or a lambda ActionRequest parsed by
a bot) into a neutral :class:`MaskState` and then call:
...
* :func:`mask_action` -- ``(action, state) -> action`` (original if valid,
  else an enforced valid replacement)
...
# bot v6) reuse this single action-id -> restriction-key map so the encoded
...
def entity_at(x: int, y: int, entities: List[dict]) -> Optional[dict]:
...
def top_asteroids(x: int, y: int, asteroids: List[dict], count: int = 5) -> List[dict]:
...
def entity_shield_state(entity: dict) -> str:
...
def active_enemies(st: MaskState) -> List[dict]:
...
def _same_zone_enemies(st: MaskState) -> List[dict]:
    ...
    return [t for t in active_enemies(st)
            if t['x'] == st.x and t['y'] == st.y]

def has_module(st: MaskState, module_name: str) -> bool:
...
def action_allowed(st: MaskState, action_name: str) -> bool:
...
def is_action_valid(action: int, st: MaskState) -> Tuple[bool, str]:
    ...
    restriction_name = ACTION_RESTRICTION_NAME.get(action)
    if restriction_name is not None:
        blocked = _restriction_reason(st, restriction_name)
        if blocked is not None:
            return False, blocked

    # Rule 4: RECHARGE END is only valid while recharging
    ...
    if action == MINE:
        ...
        asteroid = entity_at(st.x, st.y, st.asteroids)
        if asteroid is None:
